const { Chapter, SystemConfig } = require('../models/sequelize');
const publishAgent = require('../agents/publishAgent');
const aiStatus = require('./aiStatusService');

async function getPlatformConfig() {
  const row = await SystemConfig.findOne({ where: { config_key: 'publishPlatforms' } });
  if (!row) return {};
  try { return JSON.parse(row.config_value); } catch { return {}; }
}

async function publishChapter(chapterId, platforms, signal, mode = 'publish') {
  ...
  const stepLabels = platforms.map(p => `发布到${publishAgent.getPlatformList().find(x => x.key === p)?.name || p}`);
...
function parsePublishResult(raw) {
  if (!raw) return {};
